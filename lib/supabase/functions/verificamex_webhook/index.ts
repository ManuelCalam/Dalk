...
serve(async (req)=>{
  ...
  try {
    ...
    // Buscar en BD
    const { data: verification } = await supabase.from('identity_verifications').select('*').eq('verificamex_session_id', verificamexSessionId).maybeSingle();
    ...
    console.log('✅ Verificación actualizada en DB correctamente:', {
      sessionId,
      finalStatus
    });
    return new Response(JSON.stringify({
