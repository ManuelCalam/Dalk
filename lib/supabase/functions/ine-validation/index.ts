...
serve(async (req: Request) => {
  ...
  console.log('⚠️ Método no soportado:', req.method);
...
// ====================================================================
async function createVerificationSession(body: any, supabaseClient: ReturnType<typeof createClient>) {
  console.log('🏗️ ========================================');
  ...
  try {
    ...
    const generatedAccessToken = linkData.properties?.action_link
      ? linkData.properties.action_link.split('token=')[1]
    ...
    // ✅ PREPARAR URL DE REDIRECCIÓN CON TOKEN TEMPORAL
    ...
    console.log('✅ Retornando access_token temporal al cliente');

    // 🔑 RETORNAR RESPUESTA CON ACCESS_TOKEN TEMPORAL GENERADO
    return new Response(JSON.stringify({
      success: true,
      session_id: sessionId,
      form_url: formUrl,
...
// ====================================================================
async function handleVerificamexWebhook(body: any, supabaseClient: ReturnType<typeof createClient>) {
  console.log('🔔 ========================================');
