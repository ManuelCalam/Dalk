...
class DalkSupabaseUser extends BaseAuthUser {
  DalkSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> dalkSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DalkSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
